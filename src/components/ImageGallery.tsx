...
const ImageGallery: React.FC<ImageGalleryProps> = ({ images }) => {
  ...
  const renderGalleryGrid = () => {
    ...
    if (images.length === 1) {
      return (
        <div 
          className="w-full h-48 md:h-64 cursor-pointer overflow-hidden rounded-md animate-scale-up"
      ...
      return (
        <div className="grid grid-cols-2 gap-2 h-48 md:h-64">
          {images.map((image, index) => (
            <div 
              key={index}
              className="overflow-hidden rounded-md cursor-pointer animate-scale-up"
    ...
    return (
      <div className="grid grid-cols-3 gap-2">
        {/* First large image */}
        <div 
          className="col-span-2 row-span-2 h-48 md:h-64 overflow-hidden rounded-md cursor-pointer animate-scale-up"
          onClick={() => openGallery(0)}
        >
          <img 
            src={images[0]} 
        ...
        <div className="space-y-2">
          {images.slice(1, 3).map((image, index) => (
            <div 
              key={index}
              className="h-[calc(24rem/4)] md:h-[calc(16rem/2)] overflow-hidden rounded-md cursor-pointer animate-scale-up"
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-5xl w-[calc(100%-2rem)] p-0 bg-transparent border-0 shadow-none">
          <div className="relative w-full">
            ...
            {/* Image container */}
            <div className="relative flex items-center justify-center min-h-[50vh] bg-black/90 rounded-lg overflow-hidden">
              <img
